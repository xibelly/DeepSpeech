...
bool Scorer::is_scoring_boundary(PathTrie* prefix, size_t new_label)
{
  if (is_utf8_mode()) {
    ...
    int distance_to_byte = prefix->distance_to_codepoint_boundary(&first_byte);
    int needed_bytes;
    if ((first_byte >> 3) == 0x1E) {
      ...
      fprintf(stderr, "invalid byte sequence\n");
      return false;
    }
    // printf("needed bytes = %d, prefix size = %d\n", needed_bytes, distance_to_byte);
    return distance_to_byte == needed_bytes;
  } else {
