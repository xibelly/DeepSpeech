...
def dense(name, x, units, dropout_rate=None, relu=True, layer_norm=True, reuse=False):
    def relu_clip(x):
...
def rnn_impl_cudnn_rnn(x, seq_length, previous_state, reuse):
    ...
    output, output_state = fw_cell(x, sequence_lengths=seq_length, is_training=True)
...
def create_model(x, seq_length, dropout, reuse=False, previous_state=None, overlap=True, rnn_impl=rnn_impl_lstmblockcell):
    layers = {}
    ...
    layers['layer_1'] = x = dense('layer_1', x, Config.n_hidden_1, reuse=reuse)
    layers['layer_2'] = x = dense('layer_2', x, Config.n_hidden_2, reuse=reuse)
    layers['layer_3'] = x = dense('layer_3', x, Config.n_hidden_3, reuse=reuse)
    ...
    layers['layer_5'] = x = dense('layer_5', x, Config.n_hidden_5, reuse=reuse)
    layers['layer_6'] = x = dense('layer_6', x, Config.n_hidden_6, relu=False, reuse=reuse)
    layers['raw_logits'] = x
...
def calculate_mean_edit_distance_and_loss(iterator, tower, dropout, reuse):
    ...
    avg_loss = tf.reduce_mean(tf.boolean_mask(total_loss, tf.is_finite(total_loss)))
